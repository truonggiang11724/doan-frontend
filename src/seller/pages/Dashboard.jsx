        ...
        <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-600">{parseFloat(dashboard?.totalRevenue || 0).toFixed(2)} đ</p>
            </div>
            <span className="text-4xl">💰</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center pb-2 border-b">
              ...
              <span className="font-semibold text-green-600">{parseFloat(dashboard?.wallet?.availableBalance || 0).toFixed(2)} đ</span>
            </div>
            <div className="flex justify-between items-center pb-2 border-b">
              <span className="text-gray-600">Số Dư Chờ Xác Nhận</span>
              <span className="font-semibold text-yellow-600">{parseFloat(dashboard?.wallet?.pendingBalance || 0).toFixed(2)} đ</span>
            </div>
            <div className="flex justify-between items-center pb-2 border-b">
              <span className="text-gray-600">Số Dư Đã Rút</span>
              <span className="font-semibold text-blue-600">{parseFloat(dashboard?.wallet?.withdrawnBalance || 0).toFixed(2)} đ</span>
            </div>
            <div className="flex justify-between items-center pt-2 border-t">
              <span className="font-semibold text-gray-900">Total Revenue</span>
              <span className="font-bold text-lg text-green-600">{parseFloat(dashboard?.wallet?.totalRevenue || 0).toFixed(2)} đ</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center pb-2 border-b">
              ...
              <span className="font-semibold text-green-600">{parseFloat(analytics?.totalRevenue || 0).toFixed(2)} đ</span>
            </div>
            <div className="flex justify-between items-center pt-2 border-t">
              <span className="text-gray-600">Giá Trị Trung Bình Đơn Hàng</span>
              <span className="font-semibold">{parseFloat(analytics?.averageOrderValue || 0).toFixed(2)} đ</span>
            </div>
          </div>
        </div>
      </div>
              ...
              <tbody>
                {dailyRevenueData.map((row, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-right text-sm font-semibold text-green-600">{row.revenue.toFixed(2)} đ</td>
                  </tr>
                ))}
              </tbody>
            </table>
